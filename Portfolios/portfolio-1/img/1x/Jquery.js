...
// `jQuery.globalEval` so that it can fake a noncecontaining node
// via an object.
...
clone = [];
} else if ( !copyIsArray && !jQuery.isPlainObject(
src ) ) {
clone = {};
} else {
clone = src;
...
if ( nodeType === 1 &&
( rdescend.test( selector ) || rcombinators.test(
selector ) ) ) {
...
// Use (key + " ") to avoid collision with native prototype properties (seeIssue #157)
...
// https://html.spec.whatwg.org/multipage/forms.html#categorylisted
// https://html.spec.whatwg.org/multipage/forms.html#concept-fedisabled
// * option elements in a disabled optgroup
// https://html.spec.whatwg.org/multipage/forms.html#conceptoption-disabled
...
// Support: IE <=8
// Assume HTML when documentElement doesn't yet exist, such as inside loadingiframes
...
// Support: IE 8 - 11+, Edge 12 - 18+, Chrome <=16 - 25 only, Firefox <=3.6 - 31only,
...
tmp = [],
i = 0,
// By happy coincidence, a (broken) gEBTN appears onDocumentFragment nodes too
...
// https://msdn.microsoft.com/enus/library/ie/hh465388.aspx#attribute_section
...
// IE 11/Edge don't find elements on a `[name='']` query in somecases.
// Adding a temporary attribute to the document before the selectionworks
// around the issue.
...
document
// Support: IE 11+, Edge 17 - 18+
// IE/Edge sometimes throw a "Permission denied" error when strictcomparing
// two documents; shallow comparisons work.
// eslint-disable-next-line eqeqeq
...
on `parent`
if ( forward && useCache ) {
// Seek `elem` from a previouslycached index
...
++diff ) {
// Cache the
index of each encountered element
if (
useCache ) {
outerCache = node[ expando ] ||
( node[ expando ] = {} );
...
return function( elem ) {
return ( elem.textContent || getText( elem ) ).indexOf( text
) > -1;
};
} ),
...
// The matching of C against the element's language value is performed caseinsensitively.
...
// Reuse newcache so results backpropagate to previous elements
...
while ( i-- ) {
if ( ( elem = temp[ i ] ) ) {
matcherOut[ postMap[ i ] ] = !( matcherIn[ postMap[
i ] ] = elem );
}
}
}
...
// The foundational matcher ensures that elements are reachable from toplevel context(s)
...
context.nodeType === 9 && documentIsHTML && Expr.relative[ tokens[ 1
].type ] ) {
...
if ( list[ firingIndex ].apply( memory[ 0 ], memory[
1 ] ) === false &&
options.stopOnFalse ) {
...
// Support:
Promises/A+ sections 2.3.3.1, 3.5
//
...
2.3.3.3.1
// https://promisesaplus.com/#point57
...
if ( !( --remaining ) ) {
primary.resolveWith( resolveContexts,
resolveValues );
}
};
};
...
invoked if it is
// specially universal or its namespaces are a superset of
the event's.
...
// Also prevent it if we're currently inside a leveraged nativeevent stack
...
native event
// There will always be at least one argument (an
event object), so this array
// will not be confused with a leftover capture
object.
...
event.preventDefault();
// Support: Chrome 86+
// In Chrome, if an element having a
focusout handler is blurred by
// clicking outside of it, it invokes the
handler synchronously. If
// that handler calls `.remove()` on the
element, the data is cleared,
// leaving `result` undefined. We need to
guard against this.
return result && result.value;
}
// If this is an inner synthetic event for an event with a
bubbling surrogate
// (focus or blur), assume that the surrogate already
propagated from triggering the
// native event and prevent that from happening again here.
// This technically gets the ordering wrong w.r.t. to
`.trigger()` (in which the
// bubbling surrogate propagates *after* the non-bubbling
base), but that seems
...
// Support: IE <=9 - 11+
// Extend with the prototype to reset the
above stopImmediatePropagation()
...
div.style.cssText =
"position:relative;display:block;box-sizing:borderbox;overflow:scroll;" +
...
extra -
0.5
// If offsetWidth/offsetHeight is unknown, then we can't determine contentbox scroll gutter
...
// Also use offsetWidth/offsetHeight for misreported inline dimensions (gh3602)
!parseFloat( val ) && jQuery.css( elem, "display", false, styles ) ===
"inline" ) &&
// Make sure the element is visible & connected
elem.getClientRects().length ) {
isBorderBox = jQuery.css( elem, "boxSizing", false, styles ) === "borderbox";
...
//
https://web.archive.org/web/20100324014747/http://blindsignals.com/index.php/2009/07/jquerydelay/
...
//
https://web.archive.org/web/20141116233347/http://fluidproject.org/blog/2008/01/09/gettingsetting-and-removing-tabindex-values-with-javascript/
...
// https://html.spec.whatwg.org/#strip-and-collapsewhitespace
...
if ( option.selected =
jQuery.inArray( jQuery.valHooks.option.get(
option ), values ) > -1
) {
optionSet = true;
}
...
// jQuery handler
handle = ( dataPriv.get( cur, "events" ) || Object.create( null ) )[
event.type ] &&
...
// which is spec violation - http://www.w3.org/TR/DOM-Level-3-Events/#events-focuseventevent-order
 ...
 * 5) execution will start with transport dataType and THEN continue down to "*" if
needed
 */
prefilters = {},
/* Transports bindings
...
// Lazy-add the new callbacks in a
way that preserves old ones
for ( code in map ) {
statusCode[ code ] = [
...
// Put hash and anti-cache on the URL that will be requested (gh1732)
...
// Extract error from statusText and normalize for nonaborts
error = statusText;
...
only
// IE9 has no XHR2
but throws on binary (trac-11426)
// For XHR2 nontext, let the caller handle it (gh-2498)
( xhr.responseType
|| "text" ) !== "text" ||
typeof
...
// Return zeros for disconnected and hidden (display: none) elements (gh2310)
// Support: IE <=11 only
...
// Get document-relative position by adding viewport scroll to viewportrelative gBCR
